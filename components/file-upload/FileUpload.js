...
class FileUpload extends React.Component {
    ...
    fileHandler = async (event) => {
        ...
        document.getElementById("file-upload-text").innerHTML = `Upload <span class="italic underline truncate">${event.target.files[0].name}</span>`;
    }
                ...
                <div className='flex flex-col'>
                    ...
                    <label className="max-w-sm border border-black hover:bg-gray-50 hover:border-gray-300">
                        <div className="max-w-sm flex flex-col items-center justify-center pt-7">
                            ...
                            <p id="file-upload-text" className="max-w-xs truncate pt-1 text-black text-lg text-center group-hover:text-gray-600">Select Files to Upload</p>
                        </div>
                        
                        <input type="file" onChange={this.fileHandler} className="opacity-0" />
                    </label>
                    
                    <button onClick={this.fileUploadHandler} className='max-w-sm bg-white border border-black text-black font-semibold py-2 px-4 mt-8 inline-flex justify-center items-center drop-shadow hover:drop-shadow-none hover:bg-gray-50 hover:border-gray-300'>
                        <span>Upload</span>
                    </button>
                </div>
            </div>
