...
export default function Details() {
    ...
    useEffect(() => {
        if(!router.query.file) return;

        setLoading(true)
        fetch(`${process.env.serverUrl}/api/file/details/${router.query.file}`)
            .then((res) => res.json())
            .then((data) => {
                setFile(data)
                setLoading(false)
                ...
                <div className='flex flex-col'>
                    ...
                    <label className="border border-black px-10 py-2">
                        <div className="flex flex-col items-center justify-center p-1">
                            ...
                            <p className="pt-1 text-black text-lg text-center group-hover:text-gray-600">{ file.data.filename }</p>
                            <p className="pt-1 text-black text-base text-center group-hover:text-gray-600">{ file.data.size } Bytes</p>
                        </div>
                    </label>
                    ...
                    <button className='bg-white border border-black text-black font-semibold py-2 px-4 mt-8 inline-flex justify-center items-center drop-shadow hover:drop-shadow-none hover:bg-gray-50 hover:border-gray-300'>
                        <a href={`${process.env.serverUrl}/api/file/download/` + file.data.secretKey} download>Download</a>
                    </button>
                </div>
            </div>
            <div className="mt-5">
                <label className='flex'>
                    ...
                    <button onClick={ () => navigator.clipboard.writeText( process.env.clientUrl + '/details?file=' + file.data.secretKey) } >
