            ...
            <div className='mt-14 p-8 border border-black bg-red-50 border-2 border-dashed w-1/3 flex justify-center item-center'>
                <div className='flex flex-col'>
                    ...
                    <button className='bg-white border border-black text-black font-semibold py-2 px-4 mt-8 inline-flex justify-center items-center drop-shadow hover:drop-shadow-none hover:bg-gray-50 hover:border-gray-300'>
                        <a href={`http://localhost:5000/api/file/download/` + file.secretKey} download>Download</a>
                    </button>
                </div>
            </div>
            <div className="mt-5">
                <label className='flex'>
                    ...
                    <button onClick={ () => navigator.clipboard.writeText('http://localhost:3000/details?file=' + file.secretKey) } >
...
Details.getInitialProps = async (context) => {
    ...
    const key = context.query.file;

    let file = {}

    const res = await axios.get(`http://localhost:5000/api/file/details/${key}`, {
        headers: {
